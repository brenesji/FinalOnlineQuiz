...
public partial class _Default : Page
{
    ...
    //get multiple option questions
    protected void BindQuestionsMultipleOption()
    {
        DataTable dTable = new DataTable();
        SqlCommand getquestions = new SqlCommand("select id, title, data from " + quizquestionstable + " where quizid=@quizid and type='multiple' order by questionorder ASC");
    ...
    {
        DataTable dTable = new DataTable();
        SqlCommand getquestions = new SqlCommand("select id, title from " + quizquestionstable + " where quizid=@quizid and type='single' order by questionorder ASC");
        getquestions.Parameters.AddWithValue("quizid", quizId);
        ...
        if (dTable.Rows.Count > 0)
        {
            singlequestionsrpt.DataSource = dTable;
            singlequestionsrpt.DataBind();
